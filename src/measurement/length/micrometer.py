...
class MicroMeter(Meter):
    ...
    def _operate(right: object, left: object, operator: operator) -> object:
        if isinstance(right, REAL_TYPES) and type(left) == MicroMeter:
            return operator(right, left.value)
        if type(right) == MicroMeter and isinstance(left, REAL_TYPES):
    ...
    def _compare(right: object, left: object, operator: operator) -> bool:
        MicroMeter._validate(right, left, operator)
        return MicroMeter._operate(right, left, operator)
    ...
    def _math(right: object, left: object, operator: operator) -> "MicroMeter":
        MicroMeter._validate(right, left, operator)
        real = MicroMeter._operate(right, left, operator)
        if isinstance(right, REAL_TYPES) or isinstance(left, REAL_TYPES):
            return MicroMeter(real)
        return MeterConverter.convert(Meter(real), MicroMeter)
